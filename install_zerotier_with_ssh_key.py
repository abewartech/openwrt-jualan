#!/usr/bin/env python3
"""
ZeroTier Installation Script for OpenWrt with SSH Key Authentication
This script installs and configures ZeroTier on an OpenWrt router using SSH key authentication.
"""
...
@contextmanager
def ssh_connection(host, username, key_path, port=22):
    """Create an SSH connection using external SSH command with key authentication."""
    # Test SSH connection first
    test_cmd = f"ssh -i {key_path} -o StrictHostKeyChecking=no -o ConnectTimeout=15 -p {port} {username}@{host} echo 'SSH connection successful'"
    ...
    try:
        ...
        if result.returncode != 0:
            ...
            print("2. Verify your SSH key is correct and has proper permissions")
            print("3. Check if the router's SSH service is enabled and running")
            print("4. Try manually connecting with the same key: ssh -i {key_path} {username}@{host}")
            sys.exit(1)
        
        print("SSH connection successful. Using SSH key authentication for commands.")
        ...
        class ExternalSSHClient:
            ...
            def exec_command(self, command, timeout=30):
                ...
                ssh_cmd = f"ssh -i {self.key_path} -o StrictHostKeyChecking=no -o ConnectTimeout=15 -p {self.port} {self.username}@{self.host} {command}"
                ...
                try:
                    ...
                    print(f"Command timed out after {timeout} seconds. Retrying in 2s...")
                    time.sleep(2)
        ...
        client = ExternalSSHClient(host, username, key_path, port)
        yield client
    except Exception as e:
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Install and configure ZeroTier on OpenWrt using SSH key authentication')
    parser.add_argument('--host', default='192.168.1.1', help='Router IP address')
    parser.add_argument('--user', default='root', help='SSH username')
    parser.add_argument('--port', type=int, default=22, help='SSH port')
    parser.add_argument('--key', required=True, help='Path to SSH private key file')
    parser.add_argument('--network', default='56374ac9a4f632db', help='ZeroTier Network ID')
    parser.add_argument('--reboot', action='store_true', help='Reboot router after installation')
    ...
    print(f"Using ZeroTier Network ID: {args.network}")
    
    # Remove known hosts entry for the router
    remove_known_hosts_entry(args.host)
    
    # Connect to the router
    print(f"Connecting to {args.host} as {args.user} using SSH key...")
    with ssh_connection(args.host, args.user, args.key, args.port) as client:
        ...
        # Step 6: Reboot if requested
        if args.reboot:
            print("\n=== Rebooting router ===")
            run_command(client, "reboot")
            print("Router is rebooting. Please wait a few minutes for it to come back online.")
        else:
            print("\n=== Installation Complete ===")
            print("ZeroTier has been installed and configured on your OpenWrt router.")
            print(f"Network ID: {args.network}")
